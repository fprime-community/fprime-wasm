...
///
/// `verify` reads the file up front anyway — the static report needs the whole
/// thing — so this replays those bytes rather than re-reading the file.
pub struct Bytes {
    data: Vec<u8>,
    pos: usize,
...
impl Bytes {
    ...
    /// Bytes handed to the decoder so far.
